...
export const Player = () => {
  ...
  const isLooping = useGlobalStore((state) => state.isLooping);
  const toggleLoop = useGlobalStore((state) => state.toggleLoop);
      ...
      <div className="w-full max-w-[37rem]">
        <div className="flex items-center justify-center gap-6 mb-2">
          ...
          <button
            className={cn(
              "text-gray-400 hover:text-white transition-colors cursor-pointer hover:scale-105 duration-200",
              isLooping && "text-primary-400",
              !canMutate && "opacity-50 cursor-not-allowed"
            )}
            onClick={() => canMutate && toggleLoop()}
            disabled={!canMutate}
          >
            <div className="relative">
              <Repeat className={cn("size-4 relative", isLooping ? "text-primary-400" : "text-current")} />
              {isLooping && (
                <div className="absolute w-1 h-1 bg-green-500 rounded-full bottom-0 top-4.5 left-1/2 transform -translate-x-1/2 translate-y-1/2"></div>
              )}
            </div>
